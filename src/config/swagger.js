      ...
      schemas: {
        ...
        ErroValidacao: {
          ...
          properties: {
            ...
            mensagem: { type: 'string', example: 'Alguns campos estão inválidos. Verifique os detalhes abaixo.' },
            erros: {
              type: 'array',
              items: {
                type: 'object',
                properties: {
                  campo: { type: 'string', example: 'descricao' },
                  mensagem: { type: 'string', example: 'O campo "descricao" é obrigatório' }
                }
              },
              example: [
                { campo: 'descricao', mensagem: 'O campo "descricao" é obrigatório' },
                { campo: 'tipo', mensagem: 'O campo "tipo" deve ser "receita" ou "despesa"' }
              ]
            }
          },
        },
        ErroGenerico: {
          ...
          properties: {
            ...
            mensagem: { type: 'string', example: 'Ocorreu um erro inesperado. Tente novamente mais tarde.' }
          }
        },
        ErroNaoEncontrado: {
          type: 'object',
          properties: {
            ...
            mensagem: { type: 'string', example: 'A transação solicitada não foi encontrada' }
          }
        },
        ErroPermissao: {
          type: 'object',
          properties: {
            error: { type: 'string', example: 'Ação não permitida' },
            mensagem: { type: 'string', example: 'Você não tem permissão para realizar esta ação' }
          }
        },
        ErroDuplicado: {
          type: 'object',
          properties: {
            error: { type: 'string', example: 'Dados duplicados' },
            mensagem: { type: 'string', example: 'Já existe um registro com este email' }
          }
        },
      },
    },
